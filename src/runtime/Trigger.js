...
export default class Trigger {
  constructor(definition, { actions = globalThis } = {}) {
  ...
  /**
   * Create a shallow clone of this trigger with a new ID.
   * Shares the same action instance but gets its own element/selector/computedRect.
   * Used by RenderService when drawing triggers into overlay sheets
   * so the original trigger's DOM bindings are not disturbed.
   */
  clone(newId) {
   ...
   */
  async execute(controller, vars, clickedEventOrTrigger = null) {
    if (!this.action) {
