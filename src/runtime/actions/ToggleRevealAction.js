// actions/ToggleRevealAction.js
export default class ToggleRevealAction {
  // Used by the IDE to auto-render the config form and to serialize cleanly.
  static meta = {
    id: "ToggleRevealAction",
    label: "Toggle Reveal Snapshot",
    fields: [
      // What to reveal/hide
    ...
    validate(v) {
      const errors = {};
      if (!v.snapshotId) errors.snapshotId = "Pick a snapshot to toggle.";
      return errors;
    }
  };
   ...
   * Runtime entrypoint.
   * Contract:
   *  - Toggles snapshot visibility: if hidden, shows it; if visible, hides it.
   *  - Uses RenderService to handle layers/groups/scrim/z-order for showing.
   *  - Uses clearSheet for hiding.
   */
  async run(controller /*, vars */) {
    ...
    if (!snap) {
      console.warn(`[ToggleRevealAction] Snapshot not found: ${this.snapshotId}`);
      return;
    }

    // Check if snapshot is already visible
    const existingSheet = RS.findSheetBySnapshotId(this.snapshotId);
    
    if (existingSheet) {
      // Already visible - hide it
      console.log(`[ToggleRevealAction] Hiding snapshot: ${this.snapshotId}`);
      RS.clearSheet(existingSheet);
    } else {
      // Not visible - show it
      console.log(`[ToggleRevealAction] Revealing snapshot: ${this.snapshotId}`);
...
globalThis.ToggleRevealAction = ToggleRevealAction;