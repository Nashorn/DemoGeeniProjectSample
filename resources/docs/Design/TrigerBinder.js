...
export default class TriggerBinder {
  ...
  bind(triggers) {
    ...
    for (const t of triggers) {
      const el = t?.element;
      if (!(el instanceof Element) || !t?.id || this._bound.has(t.id)) continue;
 ...
 * :::How to use it (controller side):::
   ...
   // once
  this.triggerBinder = new TriggerBinder(this);

  // when ready
  this._pageDispose = this.triggerBinder.bind(pageTriggers);
  this.triggerBinder.refreshEnablement(pageTriggers);
